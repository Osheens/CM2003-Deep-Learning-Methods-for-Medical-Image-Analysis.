...
def get_data(img_path,mask_path,img_w=240,img_h=240):
    data_img = [] 
    data_mask = []
...
for s in range(max_cycles):
    ...
    train=(x_train,y_train, prior_train)
    val=(x_test,y_test,prior_test)
    train_input=(x_test,prior_test)
    ...
    model_history = model.fit(train,epochs=n_epochs,validation_data=val,verbose=1)


    

    segment_results[test_index]=model.predict(train_input, batch_size=batch_size)
